...
char *CrystalFontz635::getHardwareFirmwareVersion ( char *data ) {
    clearWriteBuffer();
    writeBuffer[0] = 0x01;
    writeBuffer[1] = 0;
    Packet *packet = sendPacket ( writeBuffer, 0x41, &dataPacket );
    if ( packet ) {
        memcpy ( data, dataPacket.data, dataPacket.length );
        data[dataPacket.length] = NULL;
        return data;
    } else {
        return NULL;
    }
...
char *CrystalFontz635::getSerial ( char *data ) {
    clearWriteBuffer();
    writeBuffer[0] = 0x01;
    writeBuffer[1] = 1;
    writeBuffer[2] = 1;
...
bool CrystalFontz635::writeUserFlash ( int8_t data[16], bool async ) {
    clearWriteBuffer();
    writeBuffer[0] = 0x02;
    writeBuffer[1] = 16;
    memcpy ( &writeBuffer[2], data, 16 );
    if ( ! async ) {
        Packet *packet = sendPacket ( writeBuffer, 0x42, &dataPacket );
        return ( packet ) ? true : false;
    } else {
        sendPacket ( writeBuffer, 0x42, NULL );
        return true;
    }
}

int8_t * CrystalFontz635::readUserFlash ( int8_t data[16] ) {
    clearWriteBuffer();
    writeBuffer[0] = 0x03;
    writeBuffer[1] = 0;
    Packet *packet = sendPacket ( writeBuffer, 0x43, &dataPacket );
    if ( ! packet ) {
        return NULL;
    }
    memcpy ( data, packet->data, 16 );
    return data;
}
...
bool CrystalFontz635::setBrightness ( uint8_t lcdBrightness, uint8_t keypadBrightness, bool async ) {
    clearWriteBuffer();
    if ( lcdBrightness > 100 ) {
        lcdBrightness = 100;
    }
    if ( keypadBrightness > 100 ) {
        keypadBrightness = 100;
    }
    writeBuffer[0] = 0x0E;
    writeBuffer[1] = 2;
    writeBuffer[2] = lcdBrightness;
    writeBuffer[3] = keypadBrightness;
    if ( ! async ) {
        Packet *packet = sendPacket ( writeBuffer, 0x4E, &dataPacket );
        return ( packet ) ? true : false;
    } else {
        sendPacket ( writeBuffer, 0x4E, NULL );
        return true; 
    }
}

bool CrystalFontz635::setContrast ( uint8_t contrast, bool async ) {
    clearWriteBuffer();
    if ( contrast > 255 ) {
        contrast = 255;
    }
    writeBuffer[0] = 0x0D;
    writeBuffer[1] = 1;
    writeBuffer[2] = contrast;
    if ( ! async ) {
        Packet *packet = sendPacket ( writeBuffer, 0x4D, &dataPacket );
        return ( packet ) ? true : false;
    } else {
        sendPacket ( writeBuffer, 0x4D, NULL );
        return true; 
    }
}
