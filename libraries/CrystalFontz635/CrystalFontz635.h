...
class CrystalFontz635 {
  public:
    ...
    char* getHardwareFirmwareVersion ( char *data );
    char* getSerial ( char *data );
    bool writeUserFlash ( int8_t data[16], bool async = false );
    int8_t * readUserFlash ( int8_t data[16] );
    ...
    bool setBrightness ( uint8_t lcdBrightness, uint8_t keypadBrightness, bool async = false );
    bool setContrast ( uint8_t contrast, bool async = false );
